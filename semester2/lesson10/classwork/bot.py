...
#     await message.answer('Привіт! Я бот - із машнами. Для реєстрації - пошир контакт', reply_markup=request_contact)

# @dp.message_handler(creditsontent_types=types.ContentTypes.CONTACT)
...
#     await message.answer(f"{full_name}, тепер напшіть ваше ім'я\nВаш номер - {phone}")
    
# @dp.message_handler(content_types='text', state=Flow.RegisterNameState)
# async def get_name(message: types.Message):
#     await Flow.RegisterState.set()
#     await message.answer(f'{message.text}, дякую за реэстрацію')
...
#                          'Тепер ви можете увімкнулти будь-яке світло:',
...
#     await message.answer('Ви увімкнули чероне світло🔴\n'
...
#                          'Тепер ви можете вимкнути сфітлофор',
